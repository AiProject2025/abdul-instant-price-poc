...
import {stateAbbreviations} from '@/utils/locationData';
import {lookupCountyByZipCode} from '@/utils/zipCodeLookup';
...
const DSCRForm: React.FC<DSCRFormProps> = ({
    ...
    useEffect(() => {
        if (initialData) {
            ...
            // Normalize state to abbreviation (e.g., "Maryland" -> "MD", "md" -> "MD", "Maryland (MD)" -> "MD")
            if (updatedData.propertyState) {
                const raw = String(updatedData.propertyState).trim();
                const displayMatch = raw.match(/^([A-Za-z\s]+)\s*\(([A-Za-z]{2})\)$/);
                if (displayMatch) {
                    updatedData.propertyState = displayMatch[2].toUpperCase();
                } else if (/^[A-Za-z]{2}$/.test(raw)) {
                    updatedData.propertyState = raw.toUpperCase();
                } else {
                    const found = Object.entries(stateAbbreviations).find(([name]) => name.toLowerCase() === raw.toLowerCase());
                    if (found) {
                        updatedData.propertyState = found[1];
                    }
                }
            }

            // Normalize county by stripping trailing suffixes and trimming
            if (updatedData.propertyCounty) {
                updatedData.propertyCounty = String(updatedData.propertyCounty)
                    .trim()
                    .replace(/\s+(County|Parish|Borough|Census Area|City|Municipality)$/i, '');
            }

            // If state/county missing but ZIP present, infer from ZIP
            if ((!updatedData.propertyState || !updatedData.propertyCounty) && updatedData.zipCode) {
                const byZip = lookupCountyByZipCode(String(updatedData.zipCode));
                if (byZip) {
                    updatedData.propertyState = updatedData.propertyState || byZip.state;
                    updatedData.propertyCounty = updatedData.propertyCounty || byZip.county;
                }
            }
    ...
    useEffect(() => {
        console.log("Testing Estimated Rehab Cost:", formState.estimatedRehabCost);
        setFormState(
            prev => ({
                    ...prev,
                    rehabCostNeeded: formState.estimatedRehabCost.toString(),
                }
            )
        )
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    <CardContent className="space-y-6">
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                            <div className="space-y-2">
                                <label
                                    className={`block text-sm font-medium ${isFieldInvalid('firstName') ? 'text-red-600' : 'text-foreground'} mb-2`}>
                                    First Name *
                                </label>
                                <Input
                                    type="text"
                            ...
                            <div className="space-y-2">
                                <label
                                    className={`block text-sm font-medium ${isFieldInvalid('lastName') ? 'text-red-600' : 'text-foreground'} mb-2`}>
                                    Last Name *
                                </label>
                                <Input
                                    type="text"
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                            <div className="space-y-2">
                                <label
                                    className={`block text-sm font-medium ${isFieldInvalid('phone') ? 'text-red-600' : 'text-foreground'} mb-2`}>
                                    Phone Number *
                                </label>
                                <Input
                                    type="tel"
                            ...
                            <div className="space-y-2">
                                <label
                                    className={`block text-sm font-medium ${isFieldInvalid('email') ? 'text-red-600' : 'text-foreground'} mb-2`}>
                                    Email Address *
                                </label>
                                <Input
                                    type="email"
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                            ...
                            <div className="space-y-2">
                                <label
                                    className={`block text-sm font-medium ${isFieldInvalid('usCitizen') ? 'text-red-600' : 'text-foreground'} mb-2`}>
                                    U.S. Citizen Status *
                                </label>
                                <Select value={formState.usCitizen}
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                            <div className="space-y-2">
                                <label
                                    className={`block text-sm font-medium ${isFieldInvalid('closingType') ? 'text-red-600' : 'text-foreground'} mb-2`}>
                                    Closing Type *
                                </label>
                                <Select value={formState.closingType}
                                        onValueChange={value => handleInputChange('closingType', value)}>
                ...
                <Card className="border-l-4 border-l-orange-500 shadow-sm">
                    ...
                    <CardContent className="space-y-6">
                        <div className="space-y-2">
                            <label
                                className={`block text-sm font-medium ${isFieldInvalid('crossCollateralLoan') ? 'text-red-600' : 'text-foreground'} mb-2`}>
                                Cross Collateral Loan *
                            </label>
                            <Select value={formState.crossCollateralLoan}
                                    onValueChange={value => handleInputChange('crossCollateralLoan', value)}>
                                <SelectTrigger
                                    className={isFieldInvalid('crossCollateralLoan') ? 'border-red-500' : ''}>
                            ...
                            {formState.crossCollateralLoan === 'Yes' && (
                                <div
                                    className="mt-4 p-4 bg-gradient-to-r from-orange-50 to-orange-100/50 border border-orange-200 rounded-lg">
                                    ...
                                    <p className="text-xs text-orange-600 mt-2">
                                        Cross collateral loans allow you to use multiple properties as collateral for a
                                        single loan.
                                    </p>
                                </div>
                            )}
                ...
                <Card className="border-l-4 border-l-blue-500 shadow-sm">
                    ...
                    <CardContent className="space-y-6">
                        <div className="space-y-2">
                            <label
                                className={`block text-sm font-medium ${isFieldInvalid('streetAddress') ? 'text-red-600' : 'text-foreground'} mb-2`}>
                                Street Address *
                            </label>
                            <Input
                                type="text"
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                            <div className="space-y-2">
                                <label
                                    className={`block text-sm font-medium ${isFieldInvalid('city') ? 'text-red-600' : 'text-foreground'} mb-2`}>
                                    City *
                                </label>
                                <Input
                                    type="text"
                            ...
                            <div className="space-y-2">
                                <label
                                    className={`block text-sm font-medium ${isFieldInvalid('zipCode') ? 'text-red-600' : 'text-foreground'} mb-2`}>
                                    ZIP Code *
                                </label>
                                <Input
                                    type="text"
                ...
                <Card>
                    ...
                    <CardContent>
                        <Select value={formState.loanPurpose}
                ...
                {formState.loanPurpose === 'Purchase' && (
                    <Card>
                        <CardHeader>
                            <CardTitle className="text-xl text-dominion-blue">Property Details - New
                                Purchase</CardTitle>
                        </CardHeader>
                        <CardContent className="space-y-6">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    <label className="block text-sm font-medium text-gray-700 mb-2">Property Type
                                        *</label>
                                    <Select value={formState.propertyType}
                                    ...
                                    {formState.propertyType === 'condominium' && (
                                        <div
                                            className="mt-4 ml-6 pl-4 border-l-2 border-blue-200 bg-blue-50/30 rounded-r-lg p-3">
                                            <label className="block text-sm font-medium text-gray-700 mb-2">
                                                <span className="text-blue-700">↳</span> Condominium Approval Type
                                            </label>
                                            <Select value={formState.condoApprovalType}
                                                    onValueChange={value => handleInputChange('condoApprovalType', value)}>
                                ...
                                <div>
                                    <label className="block text-sm font-medium text-gray-700 mb-2">Property Condition
                                        *</label>
                                    <Select value={formState.propertyCondition}
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                <div>
                                    ...
                                    <Select value={formState.rural}
                                            onValueChange={value => handleInputChange('rural', value)}>
                                        <SelectTrigger>
                                            <SelectValue placeholder="Select"/>
                                        </SelectTrigger>
                                        <SelectContent>
                                ...
                                <div>
                                    <label className="block text-sm font-medium text-gray-700 mb-2">Declining market?
                                        *</label>
                                    <Select value={formState.decliningMarket}
                                            onValueChange={value => handleInputChange('decliningMarket', value)}>
                                        <SelectTrigger>
                                            <SelectValue placeholder="Select"/>
                                        </SelectTrigger>
                                        <SelectContent>
                                ...
                                <div>
                                    <label className="block text-sm font-medium text-gray-700 mb-2">Interest only?
                                        *</label>
                                    <Select value={formState.interestOnly}
                            ...
                            <div className="space-y-4">
                                <div>
                                    <label className="block text-sm font-medium text-gray-700 mb-2">Is there a Purchase
                                        Contract in place? *</label>
                                    <Select value={formState.hasPurchaseContract}
                                            onValueChange={value => handleInputChange('hasPurchaseContract', value)}>
                                        <SelectTrigger>
                                            <SelectValue placeholder="Select"/>
                                        </SelectTrigger>
                                        <SelectContent>
                                    ...
                                    {formState.hasPurchaseContract === 'Yes' && (
                                        <div
                                            className="mt-4 ml-6 pl-4 border-l-2 border-green-200 bg-green-50/30 rounded-r-lg p-3">
                                            <label className="block text-sm font-medium text-gray-700 mb-2">
                            ...
                            <div className="space-y-4">
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div>
                                        <label className="block text-sm font-medium text-gray-700 mb-2">Number of Units
                                            *</label>
                                        <Select value={formState.numberOfUnits}
                                                onValueChange={value => handleInputChange('numberOfUnits', value)}>
                                            <SelectTrigger>
                                    ...
                                    <div>
                                        <label className="block text-sm font-medium text-gray-700 mb-2">Number of Leased
                                            Units *</label>
                                        <Input
                                            type="number"
                                ...
                                <div>
                                    <label className="block text-sm font-medium text-gray-700 mb-2">Are there any vacant
                                        units? *</label>
                                    <Select value={formState.hasVacantUnits}
                                            onValueChange={value => handleInputChange('hasVacantUnits', value)}>
                                        <SelectTrigger>
                                            <SelectValue placeholder="Select"/>
                                        </SelectTrigger>
                                        <SelectContent>
                                    ...
                                    {formState.hasVacantUnits === 'Yes' && parseInt(formState.numberOfUnits) > 1 && (
                                        <div
                                            className="mt-4 ml-6 pl-4 border-l-2 border-orange-200 bg-orange-50/30 rounded-r-lg p-3">
                                            <label className="block text-sm font-medium text-gray-700 mb-2">
                                            ...
                                            <div className="flex">
                                                ...
                                                <div className="ml-3">
                                                    <p className="text-sm text-red-700">
                                                        <strong>Bridge Product Required:</strong> Properties with more
                                                        than 10 units require our bridge financing products. Please
                                                        contact our team for specialized financing options.
                                                    </p>
                                                </div>
                                            </div>
                                        </div>
                                ...
                                {parseInt(formState.numberOfUnits) >= 2 && (
                                    <div
                                        className="ml-6 pl-4 border-l-2 border-yellow-200 bg-yellow-50/30 rounded-r-lg p-3">
                                        <label className="block text-sm font-medium text-gray-700 mb-2">
                                            <span className="text-yellow-700">↳</span> Any nonconforming units?
                                        </label>
                                        <Select value={formState.nonconformingUnits}
                                                onValueChange={value => handleInputChange('nonconformingUnits', value)}>
                                            <SelectTrigger className="bg-white">
                                                <SelectValue placeholder="Select"/>
                                            </SelectTrigger>
                                            <SelectContent>
                                ...
                                {parseInt(formState.numberOfUnits) >= 5 && (
                                    <div
                                        className="ml-6 pl-4 border-l-2 border-purple-200 bg-purple-50/30 rounded-r-lg p-3">
                                        <label className="block text-sm font-medium text-gray-700 mb-2">
                                            <span className="text-purple-700">↳</span> What is the total Net Operation
                                            Income?
                                        </label>
                                        <Input
                                            type="number"
                            ...
                            <div className="space-y-4">
                                <div>
                                    ...
                                    <Select value={formState.leaseInPlace}
                                            onValueChange={value => handleInputChange('leaseInPlace', value)}>
                                        <SelectTrigger>
                                            <SelectValue placeholder="Select"/>
                                        </SelectTrigger>
                                        <SelectContent>
                                    ...
                                    {formState.leaseInPlace === 'Yes' && (
                                        <div
                                            className="mt-4 ml-6 pl-4 border-l-2 border-indigo-200 bg-indigo-50/30 rounded-r-lg p-4 space-y-4">
                                            <div>
                                                <label className="block text-sm font-medium text-gray-700 mb-2">
                                                    <span className="text-indigo-700">↳</span> What is the lease
                                                    structure?
                                                </label>
                                                <Select value={formState.leaseStructure}
                                                        onValueChange={value => handleInputChange('leaseStructure', value)}>
                                                    <SelectTrigger className="bg-white">
                                                        <SelectValue placeholder="Select lease structure"/>
                                                    </SelectTrigger>
                                                    <SelectContent>
                                                        <SelectItem value="Long-Term Lease">Long-Term Lease</SelectItem>
                                                        <SelectItem value="Short-Term Rental">Short-Term
                                                            Rental</SelectItem>
                                                        <SelectItem value="Single-Room Occupancy">Single-Room
                                                            Occupancy</SelectItem>
                                                        <SelectItem value="Month-to-Month Lease">Month-to-Month
                                                            Lease</SelectItem>
                                                    </SelectContent>
                                                </Select>
                                            </div>
                                            ...
                                            <div>
                                                <label className="block text-sm font-medium text-gray-700 mb-2">
                                                    <span className="text-indigo-700">↳</span> Is the property under a
                                                    section 8 lease?
                                                </label>
                                                <Select value={formState.section8Lease}
                                                        onValueChange={value => handleInputChange('section8Lease', value)}>
                                                    <SelectTrigger className="bg-white">
                                                        <SelectValue placeholder="Select"/>
                                                    </SelectTrigger>
                                                    <SelectContent>
                                            ...
                                            <div>
                                                <label className="block text-sm font-medium text-gray-700 mb-2">
                                                    <span className="text-indigo-700">↳</span> Is there 12 months of STR
                                                    Rental History?
                                                </label>
                                                <Select value={formState.strRentalHistory}
                                                        onValueChange={value => handleInputChange('strRentalHistory', value)}>
                                                    <SelectTrigger className="bg-white">
                                                        <SelectValue placeholder="Select"/>
                                                    </SelectTrigger>
                                                    <SelectContent>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    <label className="block text-sm font-medium text-gray-700 mb-2">Purchase Price
                                        *</label>
                                    <Input
                                        type="number"
                ...
                {formState.loanPurpose === 'Refinance' && (
                    <Card>
                        ...
                        <CardContent className="space-y-6">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    <label className="block text-sm font-medium text-gray-700 mb-2">Refinance Type
                                        *</label>
                                    <Select value={formState.refinanceType}
                            ...
                            <div className="space-y-4">
                                ...
                                {parseInt(formState.numberOfUnits) >= 2 && (
                                    <div
                                        className="ml-6 pl-4 border-l-2 border-yellow-200 bg-yellow-50/30 rounded-r-lg p-3">
                                        <label className="block text-sm font-medium text-gray-700 mb-2">
                                            <span className="text-yellow-700">↳</span> Are these units
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <div>
                                    <label className="block text-sm font-medium text-gray-700 mb-2">Date Purchased
                                        *</label>
                                    <Input
                                        type="date"
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    <label className="block text-sm font-medium text-gray-700 mb-2">Market Value
                                        (Estimated) *</label>
                                    <Input
                                        type="number"
                            ...
                            <div className="space-y-4">
                                <div>
                                    <label className="block text-sm font-medium text-gray-700 mb-2">Is there a mortgage
                                        on the property? *</label>
                                    <Select value={formState.hasMortgage}
                                    ...
                                    {formState.hasMortgage === 'Yes' && (
                                        <div
                                            className="mt-4 ml-6 pl-4 border-l-2 border-red-200 bg-red-50/30 rounded-r-lg p-3">
                                            <label className="block text-sm font-medium text-gray-700 mb-2">
                                                <span className="text-red-700">↳</span> What is the estimated Mortgage
                                                Payoff?
                                            </label>
                                            <Input
                                                type="number"
                ...
                <Card>
                    ...
                    <CardContent className="space-y-4">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div>
                                <label className="block text-sm font-medium text-gray-700 mb-2">Desired Closing
                                    Date</label>
                                <Input
                                    type="date"
                        ...
                        {formState.loanPurpose === 'Refinance' && (
                            <div className="mt-6 p-4 bg-blue-50 border border-blue-200 rounded-lg">
                                ...
                                <Select value={formState.interestReserves}
                    ...
                    <Card>
                        <CardHeader>
                            <CardTitle className="text-xl text-dominion-blue">
                                Property Rents (Monthly)
                                - {formState.loanPurpose === 'Purchase' ? 'New Purchase' : 'Current Rents'}
                            </CardTitle>
                        </CardHeader>
                        <CardContent className="space-y-4">
                            {isPackageLoan && (
                                <div className="space-y-4">
                                    <div>
                                        <label className="block text-sm font-medium text-gray-700 mb-2">DSCR Rent
                                            (Monthly)</label>
                                        <Input
                                            type="number"
                                            ...
                                            placeholder="$0"
                                        />
                                        <p className="text-xs text-gray-500 mt-1">If provided, this overrides total unit
                                            rents for package loan pricing.</p>
                                    </div>
                                    <div>
                                        <label className="block text-sm font-medium text-gray-700 mb-2">Estimated Rent
                                            (Monthly)</label>
                                        <Input
                                            type="number"
                                    ...
                                    <div>
                                        <label className="block text-sm font-medium text-gray-700 mb-2">Market Rent
                                            (Monthly)</label>
                                        <Input
                                            type="number"
                                    ...
                                    <div className="space-y-2">
                                        <label
                                            className={`block text-sm font-medium ${isFieldInvalid('prePaymentPenaltyTerm') ? 'text-red-600' : 'text-foreground'} mb-2`}>
                                            Pre Payment Penalty Term *
                                        </label>
                                        <Select value={formState.prePaymentPenaltyTerm}
                                                onValueChange={value => handleInputChange('prePaymentPenaltyTerm', value)}>
                                            <SelectTrigger
                                                className={isFieldInvalid('prePaymentPenaltyTerm') ? 'border-red-500 focus:border-red-500' : ''}>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                                    ...
                                    {formState.totalSquareFeet && formState.numberOfUnits && (
                                        <div>
                                            ...
                                            <div
                                                className="p-3 bg-white border border-gray-300 rounded text-lg font-semibold text-dominion-blue">
                                                {Math.round(parseFloat(formState.totalSquareFeet) / parseInt(formState.numberOfUnits)).toLocaleString()} sq
                                                ft
                                            </div>
                                        </div>
                                    )}
                ...
                <Card>
                    ...
                    <CardContent className="space-y-4">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div>
                                <label className="block text-sm font-medium text-gray-700 mb-2">Annual Insurance Costs
                                    *</label>
                                <Input
                                    type="number"
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                <label className="block text-sm font-medium text-gray-700 mb-2">Annual Association
                                    Fees</label>
                                <Input
                                    type="number"
                            ...
                            <div>
                                <label className="block text-sm font-medium text-gray-700 mb-2">Annual Flood Insurance
                                    Premiums</label>
                                <Input
                                    type="number"
                ...
                <Card>
                    ...
                    <CardContent>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                <label className="block text-sm font-medium text-gray-700 mb-2">Your Credit Score
                                    *</label>
                                <Input
                                    type="number"
